...
void ArrayFireBackend::getMemMgrInfo(
    ...
    if (ostream == nullptr) {
        /*throw*/ std::invalid_argument("ArrayFireBackend::getMemMgrInfo - got null ostream pointer");
        return;
    }
    auto* curMemMgr =
...
void ArrayFireBackend::topk(
    ...
    if (axis != 0) {
        /*throw*/ std::invalid_argument("ArrayFireTensor topk: operation only supported along zero axis.");
        return;
    }
...
Tensor ArrayFireBackend::sort(
    ...
    if (sortMode != SortMode::Descending && sortMode != SortMode::Ascending) {
        /*throw*/ std::invalid_argument("Cannot sort ArrayFire tensor with given SortMode: ""only Descending and Ascending supported.");
        return Tensor();
    }
