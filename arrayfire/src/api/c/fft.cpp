...
af_array fft(const af_array in, const float norm_factor, const dim_t npad,
             const dim_t *const pad, int rank, bool direction) {
    using OutType = typename conditional<is_same<InType, float>::value ||
                                             is_same<InType, std::complex<float>>::value,
...
af_err af_fft(af_array *out, const af_array in, const float norm_factor,
              const dim_t pad0) {
    const dim_t pad[1] = {pad0};
    return fft(out, in, norm_factor, (pad0 > 0 ? 1 : 0), pad, 1, true);
}

af_err af_fft2(af_array *out, const af_array in, const float norm_factor,
...
af_err af_fft3(af_array *out, const af_array in, const float norm_factor,
...
af_err af_ifft(af_array *out, const af_array in, const float norm_factor,
               const dim_t pad0) {
    const dim_t pad[1] = {pad0};
    return fft(out, in, norm_factor, (pad0 > 0 ? 1 : 0), pad, 1, false);
}

af_err af_ifft2(af_array *out, const af_array in, const float norm_factor,
                const dim_t pad0, const dim_t pad1) {
    const dim_t pad[2] = {pad0, pad1};
    return fft(out, in, norm_factor, (pad0 > 0 && pad1 > 0 ? 2 : 0), pad, 2,
               false);
...
af_err af_ifft3(af_array *out, const af_array in, const float norm_factor,
                const dim_t pad0, const dim_t pad1, const dim_t pad2) {
    const dim_t pad[3] = {pad0, pad1, pad2};
    return fft(out, in, norm_factor, (pad0 > 0 && pad1 > 0 && pad2 > 0 ? 3 : 0),
               pad, 3, false);
...
template<typename T>
void fft_inplace(af_array in, const float norm_factor, int rank,
                 bool direction) {
...
af_err af_fft_inplace(af_array in, const float norm_factor) {
    return fft_inplace(in, norm_factor, 1, true);
}

af_err af_fft2_inplace(af_array in, const float norm_factor) {
    return fft_inplace(in, norm_factor, 2, true);
}

af_err af_fft3_inplace(af_array in, const float norm_factor) {
    return fft_inplace(in, norm_factor, 3, true);
}

af_err af_ifft_inplace(af_array in, const float norm_factor) {
    return fft_inplace(in, norm_factor, 1, false);
}

af_err af_ifft2_inplace(af_array in, const float norm_factor) {
    return fft_inplace(in, norm_factor, 2, false);
}

af_err af_ifft3_inplace(af_array in, const float norm_factor) {
    return fft_inplace(in, norm_factor, 3, false);
}

template<typename InType>
af_array fft_r2c(const af_array in, const float norm_factor, const dim_t npad,
...
af_err af_fft_r2c(af_array *out, const af_array in, const float norm_factor,
                  const dim_t pad0) {
    const dim_t pad[1] = {pad0};
    return fft_r2c(out, in, norm_factor, (pad0 > 0 ? 1 : 0), pad, 1);
}

af_err af_fft2_r2c(af_array *out, const af_array in, const float norm_factor,
                   const dim_t pad0, const dim_t pad1) {
    const dim_t pad[2] = {pad0, pad1};
...
af_err af_fft3_r2c(af_array *out, const af_array in, const float norm_factor,
                   const dim_t pad0, const dim_t pad1, const dim_t pad2) {
    const dim_t pad[3] = {pad0, pad1, pad2};
...
template<typename InType>
static af_array fft_c2r(const af_array in, const float norm_factor,
                        const dim4 &odims, const int rank) {
    using OutType = typename conditional<is_same<InType, std::complex<float>>::value,
                                         float, float>::type;
...
af_err af_fft_c2r(af_array *out, const af_array in, const float norm_factor,
                  const bool is_odd) {
    return fft_c2r(out, in, norm_factor, is_odd, 1);
}

af_err af_fft2_c2r(af_array *out, const af_array in, const float norm_factor,
                   const bool is_odd) {
    return fft_c2r(out, in, norm_factor, is_odd, 2);
}

af_err af_fft3_c2r(af_array *out, const af_array in, const float norm_factor,
                   const bool is_odd) {
