...
const int nInputCount = 6;// 7 + 9;// 19;// 43;//32
const int nOutputCount = 7;// 2 + 6;// 8;

const float learningRate = 0.0003;// 0.000001;// 0.0001;
...
struct Observation {
	...
	Vec3 OpponentGoalCenter;

	Vec3 TeamPositions[10];
	Vec3 OpponentPositions[11];
	...
	void addAngleFeature(const Vec3& heading, const Vec3& target, vector<float>& ObservationVector) {
		...
		//Vec3 side = heading_normalized.Cross(Vec3(0, 1, 0));
		Vec3 target_normalized = Vec3(target.GetX(), 0 , target.GetZ()).Normalized();
		double dot = heading_normalized.Dot(target_normalized);
		double det = heading_normalized.GetX() * target_normalized.GetZ() 
						- heading_normalized.GetZ() * target_normalized.GetX();
	...
	void addAngleFeature(const Vec3& heading, vector<float>& ObservationVector) {
		...
		double angle = atan2f(heading.GetX(), heading.GetZ());
		//angle = RadiansToDegrees(angle);
		ObservationVector.push_back(sin(angle));
		ObservationVector.push_back(cos(angle));
	}
	...
	void AddAgentObservation(const Vec3& target, vector<float>& ObservationVector) {
		...
		//Angle from agent 
		addAngleFeature(AgentHeading,  target - AgentPosition, ObservationVector);
	}
	...
	vector<float> toVector() {
		...
		//Opponent Goal
		AddAgentObservation(OpponentGoalCenter, ObservationVector);
...
struct Action {
	...
	static inline Action toAction(Tensor actionTensor) {
		...
		//action.TrackBall = tensorVec[7] > 0;
